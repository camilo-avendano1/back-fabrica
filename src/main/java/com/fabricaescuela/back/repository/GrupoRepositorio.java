...
public interface GrupoRepositorio extends JpaRepository<Grupo, Long> {
    ...
    // Nueva consulta para búsqueda + filtro por tema + paginación
    @Query(value = """
        SELECT g.nombre, 
        ...
        WHERE 
            (:search IS NULL OR (LOWER(g.nombre) LIKE LOWER(CONCAT('%', :search, '%')) 
                               OR LOWER(g.descripcion) LIKE LOWER(CONCAT('%', :search, '%'))))
